    ...
    <section className="py-12 bg-white rounded-3xl border border-[#4d7cf5]/30 p-8 shadow-xl shadow-[#4d7cf5]/10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {services.map((service) => (
          <div
            key={service.id}
            className="group relative bg-[#eef0f4] rounded-2xl p-6 transition-all duration-300 hover:-translate-y-2 hover:border-[#4d7cf5] border border-[#4d7cf5]/20 flex flex-col justify-between"
          >
            ...
            <div>
              <div className="w-12 h-12 rounded-xl bg-white border border-[#4d7cf5]/20 flex items-center justify-center mb-5 shadow-inner">
                {iconMap[service.iconName] || <Code className="w-6 h-6 text-blue-400" />}
              </div>
              <span className="text-[11px] font-bold text-[#4a5060] uppercase tracking-wider">
                {service.category}
              </span>
              <h3 className="text-xl font-bold text-[#1c1f26] mt-1 group-hover:text-[#4d7cf5] transition-colors">
                {service.title}
              </h3>
              <p className="text-[#4a5060] text-sm mt-3 leading-relaxed">
                {service.description}
              </p>

              <div className="mt-6 pt-4 border-t border-[#4d7cf5]/20 space-y-2">
                {service.deliverables.map((item, idx) => (
                  <div key={idx} className="flex items-center text-xs text-[#4a5060]">
                    <CheckCircle2 className="w-3.5 h-3.5 text-[#4d7cf5] mr-2 flex-shrink-0" />
                    <span>{item}</span>
                  </div>
                ))}
              </div>
            ...
            <div className="mt-6 pt-4">
              <Button variant="outline" size="sm" className="w-full flex items-center justify-between group-hover:bg-[#4d7cf5] group-hover:text-white group-hover:border-[#4d7cf5]">
